...
### C-01 · 라이브 지도 컴포넌트 ✅ [Phase 3] (S-08 내장, `record_run_screen.dart`)

측정 중(`_showMap = 측정중 && 좌표 존재`) S-08 상단에 `Expanded`로 실시간 경로 지도를 삽입.

| 항목 | 규칙 |
|------|------|
| 렌더 | `flutter_map` + OSM 타일(`userAgentPackageName`), 누적 폴리라인(accent) 실시간 성장 + 현재 위치 마커 |
| 카메라 | 1초 틱마다 `MapController.move(현재 좌표, 현재 줌)`로 추종(사용자 줌 보존), 레이아웃 전 호출은 무시 |
| 성능 | 폴리라인 `downsampleEvenly(points, 500)`(첫·끝 보존)로 상한, 리드로우 1Hz |
| 레이아웃 | 지도 노출 시 거리 수치 84→48dp 축소, 컨트롤은 하단 고정 |
